"""Algoritmo de Jarvis para fecho convexo."""


def pseudo_angle(v1, v2):
...
def next_point(p, i, direction):
    """Return the id of the next convex hull in the p point set"""
    lower = 8
    for j in range(0, len(p)):
        if j != i:
            angle = pseudo_angle([p[j][0] - p[i][0], p[j][1] - p[i][1]], direction)
            if angle < lower:
                lower = angle
                index = j
    return index
...
def jarvis_FC2D(p):
    ...
    next_chp = next_point(p, p.index(chp[0]), [1.0, 0.0])  # Finds the next convex
    # hull's point
    chp.append(p[next_chp])  # Adds the point to the convex hull points' set
    i = 1
    while chp[i] != chp[0]:
        direction = [chp[i][0] - chp[i - 1][0], chp[i][1] - chp[i - 1][1]]
        next_chp = next_point(p, next_chp, direction)
        chp.append(p[next_chp])
        i = i + 1
    return chp
...
plot(
    ...
    [aux[1] for aux in p],
    "bo",
    [aux[0] for aux in chp],
    [aux[1] for aux in chp],
    "k",
)
# Shows the plots
show(block=False)
